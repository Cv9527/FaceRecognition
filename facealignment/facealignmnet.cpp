...
cv::Mat FaceAlignment::getFaceChip(cv::Mat imgdata, cv::Rect rect)
{
	...
	extract_image_chip(img, get_face_chip_details(shape, 224), facechip);
...
std::vector<cv::Mat> FaceAlignment::getFaceChips(cv::Mat imgdata, std::vector<cv::Rect> rects)
{
	//将mat类型转换为dlib库支持的类型
	dlib::cv_image<dlib::rgb_pixel> img(imgdata);
	//将cv::Rect类型转换成为 dlib::rectangle类型
	...
	for (size_t i = 0; i < dets.size(); i++)
	{
		...
		dlib::array2d<dlib::rgb_pixel> facechip;
		extract_image_chip(img, get_face_chip_details(shape), facechip);
		cv::Mat tmp = dlib::toMat(facechip);
